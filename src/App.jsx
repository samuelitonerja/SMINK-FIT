...
export default function App({ userId, userEmail, cloudData }) {
  // â”€â”€ Estados inicializados desde Supabase (cloudData) â€” SIN localStorage â”€â”€
  ...
  const setUserData = useCallback(v => {
    setUserDataRaw(prev => {
      const nv = typeof v === "function" ? v(prev) : v;
      if (userId) saveProfile(userId, { email:userEmail, name:nv.name, weight:nv.weight, height:nv.height, age:nv.age, sex:nv.sex, activity:nv.activity, goal:nv.goal, num_meals:nv.numMeals, kcal_adjust:nv.kcalAdjust });
      return nv;
    });
  }, [userId, userEmail]);
  ...
  if (!profileComplete || editing) return <ProfileScreen initial={editing?userData:null} onSave={handleSaveProfile} />;
